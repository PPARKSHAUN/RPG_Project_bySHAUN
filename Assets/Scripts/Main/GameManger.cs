...
public class GameManger : MonoBehaviour
{
    ...
    public void Inventorystat()
    {
        ...
        InvenStat[4].text = "마나 : " + GameObject.FindWithTag("Player").GetComponent<CharacterManger>().stat.maxMp.ToString();
    }
    ...
    public void PointerEnter(int slotNum)//포인터가 슬롯에 들어왓을때 
    {
        ...
        if (MyItemList.Count > slotNum)
        {
            ...
            if (CurItemList[slotNum].ItemType == "Accessory" && CurItemList[slotNum].Type == "Ring")
            {
                ExplainPanel.transform.GetChild(3).GetComponent<Text>().text = CurItemList[slotNum].Explain + "\n" + "마나 + " + CurItemList[slotNum].value;// 4번째에있는 자식 text 가져와서 explain 넣음
            }
               
            else if(CurItemList[slotNum].ItemType=="Eqipment"&&CurItemList[slotNum].Type != "Wepon")
            {
                ExplainPanel.transform.GetChild(3).GetComponent<Text>().text = CurItemList[slotNum].Explain + "\n" + "체력 + " + CurItemList[slotNum].value;
            }
            else if(CurItemList[slotNum].ItemType == "Eqipment"&&CurItemList[slotNum].Type== "Wepon")
            {
                ExplainPanel.transform.GetChild(3).GetComponent<Text>().text = CurItemList[slotNum].Explain + "\n" + "공격력 + " + CurItemList[slotNum].value;
            }
            else if(CurItemList[slotNum].ItemType == "Potion"&& CurItemList[slotNum].Type =="HP")
            {
                ExplainPanel.transform.GetChild(3).GetComponent<Text>().text = CurItemList[slotNum].Explain + "\n" + "체력 " + CurItemList[slotNum].value + " 회복";
            }
            else if (CurItemList[slotNum].ItemType == "Potion" && CurItemList[slotNum].Type == "MP")
            {
                ExplainPanel.transform.GetChild(3).GetComponent<Text>().text = CurItemList[slotNum].Explain + "\n" + "마나 " + CurItemList[slotNum].value + " 회복";
            }
        }
    ...
    public void ItemDrop()
    {
        ...
        Choose(new float[3] { 1f, 1f, 98f }); // 퍼센트 조정 10 10 80 
    ...
    public void Load()
    {
        ...
        if (save == "true")
        {
            ...
            for (int i = 0; i < bro.Rows().Count; ++i)
            {
                ...
                if (jdatamyitemlist != null && jdataQuick != null)
                {
